...
func AcceptSplitBill(w http.ResponseWriter, r *http.Request) {
	...
	if err := wallet.DeductWalletBalance(uint(participant.UserID), float64(participant.Amount)); err != nil {
		http.Error(w, "failed to update wallet balance", http.StatusInternalServerError)
		return
	}
   ...
   fmt.Println("creator id",splitBill.CreatorID)
	...
	if err := wallet.UpdateWalletBalance(uint(splitBill.CreatorID), float64(participant.Amount)); err != nil {
		http.Error(w, "failed to update wallet balance", http.StatusInternalServerError)
		return
	}
